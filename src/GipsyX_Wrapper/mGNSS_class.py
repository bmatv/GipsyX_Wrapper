...
from GipsyX_Wrapper.gd2e_wrap import gd2e_class, gx_aux, gx_convert, gx_eterna, gx_ionex, gx_merge, gx_tdps, gx_trees
...
class mGNSS_class:
    ...
    def __init__(
        ...
        self.project_name = gx_aux._project_name_construct(
            project_name,
            self.PPPtype,
            self.pos_s,
            self.wetz_s,
        ...
        )  # static projects are marked as project_name_[mode]_static
        self.static_clk = static_clk
        self.staDb_path = (
            gx_aux.gen_staDb(self.tmp_dir, self.project_name, self.stations_list, self.IGS_logs_dir)
            if staDb_path is None
            else staDb_path
        )  # single staDb path for all modes.
        # Need to be able to fetch external StaDb for pbs

        self.cache_path = self.prep_cache_path(cache_path)
        self.staDb_path = (
            self.cache_staDb_path()
        )  # copy staDb  to cache. Should be cache_path/proj_name/staDb/staDb_file
    ...
    def prep_cache_path(self, cache_path):
        ...
        if not _os.path.exists(tmpX_cache_path):
            _os.makedirs(tmpX_cache_path)  # _rmtree(tmpX_cache_path)

        return tmpX_cache_path

    def cache_staDb_path(self):
        staDb_dir_cached = _os.path.join(self.cache_path, "staDb", self.project_name)
        if not _os.path.exists(staDb_dir_cached):
            _os.makedirs(staDb_dir_cached)
    ...
    def _check_PPPtype(self, PPPtype):
        ...
        if PPPtype not in PPPtypes:
            raise ValueError("Invalid PPPtype. Expected one of: %s" % PPPtypes)
        else:
            return PPPtype

    def _check_tropNom_input(self, tropNom_input):
        tropNom_inputs = ["trop", "trop+penna"]
        if tropNom_input not in tropNom_inputs:
            raise ValueError("Invalid tropNom input. Expected one of: %s" % tropNom_inputs)
    ...
    def get_drInfo(self):
        gx_aux.get_drInfo(
            num_cores=self.num_cores, tmp_dir=self.tmp_dir, tqdm=self.tqdm, selected_rnx=self.select_rnx()
        )
    ...
    def gather_mGNSS(self, force=False, stations_list=None, sigma_cut=0.05):
        ...
        if stations_list is None:
            stations_list = (
                self.stations_list
            )  # overriding stations_list so can return in chunks which saves RAM in case if large dataset
        ...
        for i in range(n_stations):
            ...
            if not _os.path.exists(filename):
                # get common index
                tmp_gps, tmp_glo, tmp_gps_glo = self._select_common(
                    gps=gps_envs[i], glo=glo_envs[i], gps_glo=gps_glo_envs[i]
                )
    ...
    def gather_aux(self, gps_only=False, force=False):
        ...
        for i in range(len(self.stations_list)):
            filename = _os.path.join(
                aux_dir, "{}_{}_aux{}.zstd".format(self.project_name, self.stations_list[i].lower(), suffix)
            )
            if force:
                if _os.path.exists(filename):
                    _os.remove(filename)
            if _os.path.exists(filename):
                ...
                for project in project_list:  # read per constellation for this station
                    ...
                    columns = columns_filtered[
                        _np.isin(columns_filtered.to_series().str.split(".", expand=True)[3], ["Trop", "Clk"])
                    ]  # names of Trop and Clk columns to extract
                    station_buf.append(
                        gx_aux._update_mindex(filtered_solution.loc(axis=1)[:, columns], self.stations_list[i].upper())
                    )
                if gps_only:
                    tmp = station_buf
                else:
    ...
    def analyze(
        self, gps_only, restore_otl=True, remove_outliers=True, sampling=1800, force=False, begin=None, end=None
    ):
        ...
        if force:  # if force - remove gather file!
            if _os.path.exists(gather_path):
                _os.remove(gather_path)
        ...
        if not _os.path.exists(gather_path):
            if gps_only:
                ...
                tmp_gps = self.gps.analyze_env(
                    force=force, mode="GPS", restore_otl=restore_otl, begin=begin_date, end=end_date
                )
                ...
                for stations_sublist in stations_sublists:
                    ...
                    tmp_synth = self.gps.analyze_env(
                        envs=envs, force=force, mode="GPS", otl_env=True, begin=begin_date, end=end_date
                    )
                    tmp_gps = self.gps.analyze_env(
                        envs=envs, force=force, mode="GPS", restore_otl=restore_otl, begin=begin_date, end=end_date
                    )
                    tmp_glo = self.glo.analyze_env(
                        envs=envs, force=force, mode="GLONASS", restore_otl=restore_otl, begin=begin_date, end=end_date
                    )
                    tmp_gps_glo = self.gps_glo.analyze_env(
                        envs=envs,
                        force=force,
                        mode="GPS+GLONASS",
                    ...
                    tmp_blq_concat_partial = _pd.concat(
                        [tmp_synth, tmp_gps, tmp_glo, tmp_gps_glo],
                        keys=["OTL", "GPS", "GLONASS", "GPS+GLONASS"],
                        axis=1,
                    )
            ...
            gx_aux._dump_write(
                data=tmp_blq_concat, filename=gather_path, num_cores=2, cname="zstd"
            )  # dumping to disk mGNSS eterna gather
    ...
    def analyze_aux(
        self, v_type="value", parameter="WetZ", sampling=1800, force=False, begin=None, end=None, gps_only=False
    ):
        ...
        if parameter not in parameters:
            raise ValueError("Unexpected parameter {}".format(parameter))
        ...
        if not _os.path.exists(gather_path):
            ...
            for project in project_list:
                tmp.append(
                    project.analyze_wetz(
                        wetz_gather=wetz_gather,
                        parameter=parameter,
                        force=force,
                        begin=begin_date,
                        end=end_date,
