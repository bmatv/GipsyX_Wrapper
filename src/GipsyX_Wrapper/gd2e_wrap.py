...
from GipsyX_Wrapper.gxlib import (
    gx_aux,
    gx_compute,
    gx_convert,
    gx_eterna,
...
class gd2e_class:
    def __init__(
        ...
        self.project_name = project_name + gx_aux.mode2label(
            mode=self.mode
        )  # UPDATING PROJECT NAME DEPENDING ON THE MODE
    ...
    def solutions(self, single_station=None):
        return gx_extract.gather_solutions(
            num_cores=self.num_cores,
            project_name=self.project_name,
            stations_list=self.stations_list,
            tmp_dir=self.tmp_dir,
    ...
    def residuals(self, single_station=False):
        return gx_extract.gather_residuals(
            num_cores=self.num_cores,
            project_name=self.project_name,
            stations_list=self.stations_list,
            tmp_dir=self.tmp_dir,
    ...
    def envs(self, sigma_cut=0.05, dump=False, force=False, stations_list=None):
        ...
        env_gather_path = _os.path.join(
            self.tmp_dir, "gd2e/env_gathers", self.project_name_core
        )  # saning to core where all mGNSS env_gathers are located
        if not _os.path.exists(env_gather_path):
            _os.makedirs(env_gather_path)
        envs = _np.ndarray((len(stations_list)), dtype=object)
        incomplete = False
        for i in range(envs.shape[0]):
            env_path = _os.path.join(
                env_gather_path, "{}{}.zstd".format(stations_list[i].lower(), gx_aux.mode2label(self.mode))
            )  # naming convention as site_gps.zstd
            if force and _os.path.exists(env_path):
                _os.remove(env_path)
            if _os.path.exists(env_path):
                ...
                break
        if incomplete:
            envs = gx_aux._xyz2env(
                dataset=self.filtered_solutions(sigma_cut=sigma_cut),  # filtered_solutions takes most of the time
                reference_df=self.refence_xyz_df,
                mode=self.mode,
                dump=env_gather_path if dump else None,
            )
        return envs
    ...
    def analyze_wetz(
        ...
        if (
            wetz_gather is None
        ):  # wetz_gather may be fascilitated by mGNSS class so different cionstellation solutions will be in sync
            wetz_gather = _np.ndarray((len(self.stations_list)), dtype=object)
            for station in self.stations_list:
                wetz_array = self.filtered_solutions(
                    single_station=station
                )  # ideally a function that can dump wetz gathers. Need to make it concurrent per station
                ...
                wetz_gather[i] = gx_aux._update_mindex(
                    gx_aux._update_mindex(tmp, self.stations_list[i].upper()), self.mode
                )
