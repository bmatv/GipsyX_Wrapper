...
def _filter_sigma(dataset, sigma_cut):
...
def filter_tdps(tdps, sigma_cut=0.1):
    # std_coeff=3 is not used as not efficient. Used at eterna stage
    if type(tdps) == _pd.core.frame.DataFrame:
        clk_filter = _filter_clk(
            dataset=tdps,
        )
        filtered_tdps = _filter_sigma(dataset=clk_filter, sigma_cut=sigma_cut)
        print(
            "Clk.Bias Filter: {:.2f} left. Sigmas Filter: {:.2f} left.".format(
                clk_filter.shape[0] / tdps.shape[0] * 100, filtered_tdps.shape[0] / tdps.shape[0] * 100
            )
        )
    else:
        ...
        for i in range(tdps.shape[0]):
            ...
            sigma_filter = _filter_sigma(dataset=clk_filter, sigma_cut=sigma_cut)
            filtered_tdps[i] = sigma_filter

            # sigma_filter = _filter_sigma(dataset = clk_filter, std_coeff=std_coeff)
            print(
                "Clk.Bias Filter: {:.2f} left. Sigmas Filter: {:.2f} left.".format(
                    clk_filter.shape[0] / tdps[i].shape[0] * 100, sigma_filter.shape[0] / tdps[i].shape[0] * 100
                )
            )
...
def _gen_windows(dataset):
    ...
    time_frame_in = (
        dataset.index[[0, -1]].values.astype("timedelta64") + J2000origin
    )  # Getting first and last time values of the dataset (pandas index)
...
def _stretch(dataset, sampling=300):
    ...
    df_theoretical = _pd.DataFrame(
        index=_np.arange(timewindow[0], timewindow[1], sampling)
    )  # input sampling of the dataset(5 mins)
...
def _avg_30(dataset):
    ...
    first_elements_rolled = _np.roll(
        dataset_reshaped[:, 0, :], shift=-1, axis=0
    )  # rolling up. So first element becomes last element of previous timeframe
    first_elements_rolled_reshaped = first_elements_rolled.reshape(
        (first_elements_rolled.shape[0], 1, first_elements_rolled.shape[1])
    )
    ...
    with warnings.catch_warnings():
        ...
        avg_30 = _pd.DataFrame(
            _np.nanmean(dataset_reshaped_centered, axis=1), index=corrected_time_series, columns=dataset.columns
        )
    return avg_30
