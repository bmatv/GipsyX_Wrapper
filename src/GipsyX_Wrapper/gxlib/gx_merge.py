...
def get_merge_table(tmp_dir, stations_list, mode=None):
    ...
    if mode is None:
        ...
        complete_record = drinfo[
            (drinfo["GPS"] > 0) & (drinfo["GLONASS"] > 0)
        ]  # at least one satellite of each constellation for the processing
    ...
    for station in stations_list:
        ...
        if (drinfo_rec_time > 24).sum() != 0:
            print("Files longer than 24 hours detected in drInfo. Ignoring those as are possibly corrupted.")
        ...
        completeness[(drinfo_rec_time >= 20) & ((drinfo_rec_time) < 24)] = (
            2  # as we work with 24 daily files, we do not use files that are longer
        )
        # -----------------------------------------------------------------------
        ...
        start_c_day = station_start64.astype(
            "datetime64[D]"
        )  # this values should overwrite begin, otherwise duplicates may appear as if begin YYYY-MM-DD 02:25:00 + 27 !!! 05:25:00
        ...
        B1c1 = (start_c_day - start_p_hour <= _np.timedelta64(24, "[h]")) & (
            start_c_day - start_p_hour >= _np.timedelta64(3, "[h]")
        )
        ...
        B2c1 = (end_n_hour - start_c_day <= _np.timedelta64(48, "[h]")) & (
            end_n_hour - start_c_day >= _np.timedelta64(27, "[h]")
        )  # start_c_day is the same as end_c_day

        B2c2 = (start_n_hour - start_c_day <= _np.timedelta64(25, "[h]")) & (
...
def _merge(merge_set):
    ...
    drMerge_proc = _Popen(
        [
            "drMerge",
            str(merge_set["merge_begin"]),
            str(merge_set["merge_end"]),
            _os.path.basename(merge_set["path"]) + ".30h",
            merge_set["path_prev"],
            merge_set["path"],
            merge_set["path_next"],
        ],
...
def dr_merge(merge_table, num_cores, tqdm):
    ...
    df_class3["merge_begin"] = (
        (df_class3["begin"].astype("datetime64[D]") - _np.timedelta64(3, "[h]") - J2000origin)
        .astype("timedelta64[s]")
        .astype(int)
    )
    df_class3["merge_end"] = (
        (df_class3["begin"].astype("datetime64[D]") + _np.timedelta64(27, "[h]") - J2000origin)
        .astype("timedelta64[s]")
        .astype(int)
    )
    ...
    if (merge_table_class3[~ifexists]).shape[0] == 0:
        ...
        with _Pool(processes=num_cores) as p:
            if tqdm:
                list(
                    _tqdm.tqdm(
                        p.imap(_merge, merge_table_class3_run.to_records()), total=merge_table_class3_run.shape[0]
                    )
                )
            else:
