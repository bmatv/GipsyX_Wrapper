...
from GipsyX_Wrapper.gxlib.gx_aux import J2000origin as _J2000origin
from GipsyX_Wrapper.gxlib.gx_aux import date2yyyydoy
from GipsyX_Wrapper.gxlib.gx_filter import _avg_30, _stretch
...
def _write_ETERNA(dataset, filename, sampling, station_name):
    ...
    for i in range(len(block_ends)):
        ...
        if block.shape[0] >= 24:
            block_begin = (
                "{}               1.0000    1.0000     0.000         0    BLOCK{}\n77777777            0.000\n".format(
                    station_name, block_number
                )
            )

            out_buf += (
                block_begin
                + data.iloc[block_begin_ind:block_end_ind].to_string(
                    columns=["Date_et", "Time_et", "Data"],
                    formatters={"Date_et": a, "Time_et": b, "Data": c},
                    index=False,
                    header=False,
                )
                + block_end
...
def _interp_short_gaps(dataset_avg):
    ...
    dataset_breaks["break_length"] = (
        (dataset_breaks["break_end"] - dataset_breaks["break_begin"]) - 1800
    ) / 3600  # how many hours is in the break
    short_breaks = dataset_breaks[
        (dataset_breaks["break_length"] <= 12) & (dataset_breaks["break_length"] > 0)
    ]  # these are short breaks
...
def env2eterna(dataset, remove_outliers, v_type="value"):
    ...
    if remove_outliers:
        filt1 = _remove_outliers(dataset, v_type=v_type)  # Turning off and on the detrending
    else:
        filt1 = dataset[v_type]
...
def get_staDb_llh(staDb_path):
    ...
    llh_stdb = _np.asarray(staDb.dumpLatLonHeights(epoch=max_t, stationList=stns))
...
def analyse_et(
    ...
    dir_name = "{}_{}_{}".format(
        "tmp_otl_et" if otl_env else "tmp_et", date2yyyydoy(begin_date), date2yyyydoy(end_date)
    )
    ...
    if env_et.shape[-1] == 3:
        ...
        components = [
            "aux_eter"
        ]  # used to be wetz_ete. renamed to aux_eter. All single column dataseries are treated as aux analysis timeseries

    components_exist = []
    for component in components:
        if force:  # force removes eterna tmp folders
            print('Using "force" option', end=" | ")
            if _os.path.exists(_os.path.join(tmp_station_path, component)):
                _shutil.rmtree(_os.path.join(tmp_station_path, component))
    ...
    if not eterna_exists:
        print(
            ("Processing {} {} with Eterna {}-{} | {}").format(station_name, mode, begin_date, end_date, dir_name),
            end=" | ",
        )
        ...
        for i in range(env_et.shape[-1]):
            ...
            _write_ETERNA(
                ...
                filename=_os.path.join(comp_path, components[i] + ".dat"),
                sampling=1800,
...
def read_prn(prn_file):
    ...
    df = _pd.read_fwf(
        ...
        sep="\n",
        na_values="*********",
        colspecs=[(0, 14), (15, 23), (23, 28), (28, 37), (38, 47), (47, 56), (56, 65), (65, 76)],
        names=["from", "to", "wave", "theor_a", "a_factor", "a_stdv", "phase", "phase_stdv"],
...
def extract_et(
    ...
    df_blq = _pd.DataFrame(
        columns=columns_mlevel, index=["M2", "S2", "N2", "K2", "K1", "O1", "P1", "Q1", "MF", "MM", "SSA", "14h"]
    )
...
def analyze_env_single_thread(values_set):
    ...
    if parameter is not None:
        ...
        parameter = [
            "{}.{}".format("Trop", parameter)
            if parameter in Trop
            else "Clk.Bias"
            if parameter == "Clk"
            else ValueError("Unexpected parameter {}".format(parameter))
        ]
        env = env.loc(axis=1)[station_name, :, ".Station.{}.{}".format(station_name.upper(), parameter[0])]

    begin_J2000 = (
        (env.index[0]) if begin_date is None else (begin_date - _J2000origin).astype("timedelta64[s]").astype(int)
    )
    ...
    env_et = env2eterna(
        env_station[(env_station.index >= begin_J2000) & (env_station.index <= end_J2000)],
        remove_outliers,
        v_type=v_type,
    )

    if otl_env:
        synth_otl = gen_synth_otl(
            dataset=env_et, station_name=station_name, hardisp_path=hardisp_path, blq_file=blq_file, sampling=sampling
        )
        blq_array = _pd.concat(
            [
                analyse_et(
                    env_et=synth_otl,
