...
from .gx_aux import J2000origin, _check_stations, _dump_read, _dump_write
...
def gather_solutions(tmp_dir, project_name, stations_list, num_cores, tqdm, single_station=None):
    ...
    if single_station is None:
        ...
        paths_tmp = (
            tmp_dir + "/gd2e/" + project_name + "/" + _np.asarray(checked_stations, dtype=object) + "/solutions.zstd"
        )
        ...
        for i in range(n_stations):
            if not _os.path.exists(paths_tmp[i]):
                print(
                    "No gather file for {} station in {}.\n Running extract_tdps for the dataset.".format(
                        checked_stations[i], project_name
                    )
                )
...
def gather_residuals(tmp_dir, project_name, stations_list, num_cores, tqdm, single_station=False):
    ...
    # Create a list of paths to get data from
    paths_tmp = (
        tmp_dir + "/gd2e/" + project_name + "/" + _np.asarray(checked_stations, dtype=object) + "/residuals.zstd"
    )
...
def extract_tdps(tmp_dir, project_name, station_name, num_cores, tqdm):
    ...
    station_files = _np.asarray(
        sorted(_glob.glob(tmp_dir + "/gd2e/" + project_name + "/" + station_name + "/*/*.zstd"))
    )
...
def _gather_tdps(station_files, num_cores, tqdm):
    ...
    with _Pool(processes=num_cores) as p:
        if tqdm:
            data = list(
                _tqdm.tqdm(p.imap(_get_tdps_npz, station_files, chunksize=chunksize), total=station_files.shape[0])
            )
        else:
            data = p.map(_get_tdps_npz, station_files, chunksize=chunksize)
    return data
...
def _get_tdps_npz(file):
    ...
    residuals = tmp_residuals[
        (tmp_residuals.index.get_level_values(1) >= begin_timeframe)
        & (tmp_residuals.index.get_level_values(1) < end_timeframe)
    ]
