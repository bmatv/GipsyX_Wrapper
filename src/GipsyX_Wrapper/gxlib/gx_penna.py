...
def get_debug(tmp_dir, project_name):
    ...
    npz1_path = sorted(
        _glob.glob(
            (
                "{}/gd2e/{}/{}/{}/*/gipsyx_out.npz".format(
                    tmp_dir, project_name, stations_list[0], years_list_station_0[0]
                )
            )
        )
    )[0]
    ...
    coordinate_processing = (
        state_stoc_adj.str.split(expand=True)[[1, 2, 4]]
        .rename(columns={1: "coord_apriori_sigma", 2: "coord_procnoise_sigma", 4: "coord_mode"})
...
def STD_WetZ(kin_solutions_filtered_avg, sta_solutions_filtered_avg):
    ...
    for i in range(std_array.shape[0]):
        ...
        std_array[i] = (
            kin_solutions_filtered_avg[i]["value"].iloc[:, wetz_index_kinematic]
            - sta_solutions_filtered_avg[i]["value"].iloc[:, wetz_index_static]
        ).std() * 1000
    return std_array
