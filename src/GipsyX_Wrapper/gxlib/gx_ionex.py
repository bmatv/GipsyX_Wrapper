...
class ionex:
    ...
    def __init__(
        ...
        cache_path,
        tqdm,
    ):  # we take cache_path as files will be copied and decompressed before read
    ...
    def _extended_list(self):
        ...
        if self.ionex_type == "jpl":  # jpl is jpl_native by default
            path_series = _pd.Series(
                sorted(
                    _glob.glob(_os.path.abspath(_os.path.join(self.ionex_prods_dir, "y[1-2][0-9][0-9][0-9]/JPLG*.gz")))
                )
            )  # assure we take .Z files
            ...
            path_series = _pd.Series(
                sorted(
                    _glob.glob(
                        _os.path.abspath(
                            _os.path.join(self.ionex_prods_dir, "[1-2][0-9][0-9][0-9]/*", self.ionex_type + "*.Z")
                        )
                    )
                )
            )  # assure we take .Z files
    ...
    def _get_ionex_list(self, year):
        ...
        if (
            len(extended_list[extended_list.iloc[:, 0] == year - 1])
            == 0 & len(extended_list[extended_list.iloc[:, 0] == year + 1])
            == 0
        ):
            list_files_out = extended_list[extended_list.iloc[:, 0] == year]

        if len(extended_list[extended_list.iloc[:, 0] == year - 1]) == 0:
            list_files_out = _pd.concat(
                (
        ...
        if len(extended_list[extended_list.iloc[:, 0] == year + 1]) == 0:
            list_files_out = _pd.concat(
                (
                    extended_list[extended_list.iloc[:, 0] == year - 1].tail(1),
                    extended_list[extended_list.iloc[:, 0] == year],
                )
            )
        else:
    ...
    def _GIM_gen_header(self, in_set, data_GIM_final):
        ...
        with open(last_file_path, "rb") as ionex_last:
            ...
            match_last_epoch = _re.search(
                regex_first_epoch, ionex_data_last
            )  # EPOCH OF LAST MAP (LAST FILE to continue the header as headers can have different line quantity)
        ...
        return (
            (
                ionex_data_first[: match_first_epoch.end()]
                + ionex_data_last[match_last_epoch.end() : match_num_maps_b.end()]
            ).decode("ascii")
            + num_maps
            + (
                ionex_data_last[match_num_maps_e.end() : match_aux_begin.start()]
                + ionex_data_last[match_aux_end.end() : match_end_header.end()]
            ).decode("ascii")
        )
