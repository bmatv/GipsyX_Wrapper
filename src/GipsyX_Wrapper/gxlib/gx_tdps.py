...
def gen_tropnom(tmp_dir, stadb_path, rate, vmf1_dir, num_cores):
    ...
    logger.info(
        len(stns), "sites found in staDb:", stns
    )  # verbal output of stations that will be present in tropNom files
    ...
    for year in drinfo_years_list:
        ...
        tropNom_out = (
            ...
            + str(year)
            + "/"
            + _pd.Series(date).dt.dayofyear.astype(str).str.zfill(3)
            + "/30h_tropNominalOut_VMF1.tdp"
        ).values
        ...
        staDb_nd.fill(stadb)
        rate_nd.fill(rate)
        VMF1_dir_nd.fill(vmf1_dir)
        stns_nd.fill(stns)

        tropnom_param = _np.column_stack(
            (datetime_to_j2000(begin), datetime_to_j2000(end), tropNom_out, staDb_nd, rate_nd, VMF1_dir_nd, stns_nd)
        )
...
def write_tdp(output_file, tdp_concat):
    ...
    with open(output_file, "w") as file:
        file.write(
            tdp_concat.to_string(
                columns=["Time", "NominalValue", "Value", "Sigma", "Name"],
                formatters={
                    "Time": tdp_num,
...
def _gen_penna_tdp_file(np_set):
    ...
    tropNom_table = _pd.read_csv(
        path2tdp_file, delim_whitespace=True, header=None, names=["Time", "NominalValue", "Value", "Sigma", "Name"]
    )  # reading tdp file with pandas
...
def gen_penna_tdp(
    ...
    aux = _np.empty(
        (files.shape[0], 6), dtype=object
    )  # population array [xyz_staDb_data, period, A_East, A_North, A_Vertical, rot]
    ...
    with _Pool(processes=num_cores) as p:
        if tqdm:
            list(_tqdm.tqdm(p.imap(_gen_penna_tdp_file, np_set), total=np_set.shape[0]))
        else:
            p.map(_gen_penna_tdp_file, np_set)
