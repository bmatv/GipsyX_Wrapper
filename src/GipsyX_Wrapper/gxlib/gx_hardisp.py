...
def blq2hardisp(blq_file):
    ...
    sites = (_pd.Series(blq_file_read.reshape((n_stations, 7))[:, 0])).str.strip()

    blq_raw = _pd.Series(blq_file_read.reshape((n_stations, 7))[:, 1:].reshape((n_stations * 6)))
...
def reformat_blq(blq_string):
    ...
    amplitude = (
        df.iloc[:3].round(5).to_string(header=None, index=None, float_format="%.5f")
    )  # rounding values and string conversion round(5)
    amplitude = (
        _pd.Series(amplitude)
        .str.split("\n", expand=True)
        .T.squeeze()
...
def gen_synth_otl(dataset, station_name, hardisp_path, blq_file, sampling):
    ...
    synth_otl = (
        _pd.read_csv(out, error_bad_lines=False, header=None, delim_whitespace=True, names=["dU", "dS", "dW"]) * 1000
    )  # convert to mm as returns in m
    ...
    tmp["east"] = (
        synth_otl["dW"] * -1
    )  # Conversion to dE. Already checked that it is a correct way (phases) as same as GipsyX does correction (no otl - otl_corrected)
