...
def _2dr(rnx2dr_path):
    ...
    process = _Popen(
        ["rnxEditGde.py", "-dataFile", in_file_cache_path, "-staDb", staDb_path, "-o", out_file_cache_path],
        cwd=cache_dir,
    )
    process.wait()
...
def rnx2dr(selected_df, num_cores, tqdm, cache_path, staDb_path, cddis=False):
    ...
    selected_df2convert["staDb_path"] = (
        staDb_path  # populating with staDb_path which is needed as rnx files may lack receiver information
    )
    ...
    if selected_df2convert.shape[0] > 0:
        ...
        with _Pool(processes=num_cores) as p:
            if tqdm:
                list(_tqdm.tqdm(p.imap(_2dr, selected_df2convert), total=selected_df2convert.shape[0]))
            else:
