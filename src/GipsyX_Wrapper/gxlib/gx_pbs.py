...
def qsub_python_code(code, name, email, cleanup, pbs_base, walltime="48:00:00"):
    ...
    with open(pbs_script_path, "w") as pbs_script:
        pbs_script.write(
            TEMPLATE_SERIAL.format(name=name, logfile_path=logfile_path, email=email, code=code, walltime=walltime)
        )
...
def gen_code(
    ...
    rate,
    gnss_products_dir,
    hatanaka,  # we should use COD MGEX, ESA and GFZ later
    eterna_path,
    hardisp_path,
    rnx_dir,
    cddis,
