...
_regex_ID = _re.compile(
    r"1\.\W+S.+\W+Site Name\s+\:\s(.+|)\W+Four Character ID\s+\:\s(.+|)\W+Monument Inscription\s+\:\s(.+|)\W+IERS DOMES Number\s+\:\s(.+|)\W+CDP Number\s+\:\s(.+|)",
    _re.MULTILINE,
)
_regex_loc = _re.compile(
    r"2\.\W+S.+\W+City or Town\W+\:\s(.+|)\W+State or Province\W+\:\s(.+|)\W+Country\W+\:\s(.+|)\W+Tectonic Plate\W+\:\s(.+|)\W+.+\W+X.+\:\s(.+|)\W+Y..+\:\s(.+|)\W+Z.+\:\s(.+|)\W*Latitude.+\:\s(.+|)\W*Longitude.+\:\s(.+|)\W*Elevation.+\:\s(.+|)",
    _re.MULTILINE,
)
_regex_rec = _re.compile(
    r"3\.\d+\s+R.+\W+\:\s(.+|)\W+Satellite System\W+\:\s\(?(?:\(\w*\/\w*\/|)([a-zA-Z+]+)\W+Serial Number\W+\:\s(.+|)\W+Firmware Version\W+\:\s(.+|)\W+Elevation Cutoff Setting\W+\:\s(.+|)\W+Date Installed\W+\:\s(\d{4}.{1}\d{2}.{1}\d{2}|)(?:.{1}|)(\d{2}.{1}\d{2}|)Z?\W+Date Removed\W+\:\s(\d{4}.{1}\d{2}.{1}\d{2}|)(?:.{1}|)(\d{2}.{1}\d{2}|)",
    _re.MULTILINE,
)
_regex_ant = _re.compile(
    r"4\.\d\s+Antenna Type.+\W+:\s(\w+\S?\w+?|)\s+(\w+|)\W+Serial Number\W+:\s(\w+\s?\W?\w+?|)\W+Antenna.+:\s(.+|)\W+Marker->ARP Up.+:\s(.+|)\W+Marker->ARP North.+:\s(.+|)\W+Marker->ARP East.+:\s(.+|)\W+Alignment from True N\W+:\s(.+|)\W+Antenna Radome Type\W+:\s(.+|)\W+Radome Serial Number\W+:\s(.+|)\W+Antenna Cable Type\W+:\s(.+|)\W+Antenna Cable Length\W+:\s(.+|)\W+Date Installed\W+:\s(.{10})T?(.{5}|)Z?\W+Date Removed\W+\:(?:\s\(?(.{10})T(.{5}|)Z?|)\W+Additional Information\W+:\s(.+|)\W+",
    _re.MULTILINE,
)
...
def prepare_dir_struct_dr(begin_year, end_year, tmp_dir):
    timeline = _pd.Series(
        _np.arange(_np.datetime64(str(begin_year)), _np.datetime64(str(end_year + 1)), _np.timedelta64(1, "D"))
    )
...
def _project_name_construct(
    project_name, PPPtype, pos_s, wetz_s, tropNom_input, ElMin, ambres, tree_options=trees_options.rw_otl
):
...
def gen_staDb(tmp_dir, project_name, stations_list, IGS_logs_dir):
    ...
    for i in range(len(stations_list)):
        logs[i] = sorted(_glob.glob(IGS_logs_dir + "/*/" + stations_list[i].lower() + "*" + ".log"))[
            -1
        ]  # should be the last log created in case multiple exist
    ...
    for file in logs:
        ...
        buf += "{ID}  ID  {IERS} {loc_2} {loc_1}\n".format(
            ID=matches_ID[0][1],
            IERS=matches_ID[0][3] if matches_ID[0][3] != "" else "UNKNOWN",
            loc_2=matches_loc[0][1],
            loc_1=matches_loc[0][2],
        )
        ...
        for matchNum, match in enumerate(matches_rec):
            ...
            buf += "{ID}  RX {d_inst} {t_inst}:00 {rec_type} # {rec_num} {rec_fw_v}\n".format(
                ID=matches_ID[0][1],
                d_inst=rec[matchNum][5],
                t_inst=rec[matchNum][6] if rec[matchNum][6] != "" else "00:00",
                rec_type=rec[matchNum][0],
                rec_num=rec[matchNum][2],
                rec_fw_v=rec[matchNum][3],
            )
        # Antenna Information
        ant = []
        ...
        for matchNum, match in enumerate(matches_ant):
            ...
            buf += "{ID}  ANT {d_inst} {t_inst}:00 {ant_type} {radome_type} {east} {north} {vertical} # {ant_num}\n".format(
                ID=matches_ID[0][1],
                d_inst=ant[matchNum][12],
                t_inst=ant[matchNum][13] if ant[matchNum][13] != "" else "00:00",
                ant_type=ant[matchNum][0],
                radome_type=ant[matchNum][8] if ant[matchNum][8] != "" else "NONE",
                vertical=ant[matchNum][4],
                north=ant[matchNum][5],
                east=ant[matchNum][6],
                ant_num=ant[matchNum][2],
            )
...
def get_chalmers(staDb_path, as_df=False):
    ...
    max_t = (
        3.0e8  # maximum time value for the dataset on which available sites will be added to OTL computation with SPOTL
    )
    names_stdb = _np.asarray(staDb.getStationList(), dtype=object)
    llh_stdb = _np.asarray(staDb.dumpLatLonHeights(epoch=max_t, stationList=staDb.getStationList()))
...
def _drInfo2df(dr_file):
    ...
    drInfo_process = _Popen(
        args=["dataRecordInfo", "-file", _os.path.basename(dr_file)],
        stdout=_PIPE,
        stderr=_STDOUT,
    ...
    df["GLONASS"] = transmitters.str.contains(
        pat=r"R\d{3}"
    ).sum()  # number of GLONASS satellites present in the dr file
    ...
    for station in stations:
        for year in years:
            filename = "{drinfo_dir}/{yyyy}/{station}{yy}.zstd".format(
                drinfo_dir=drinfo_dir, yyyy=year.astype(str), station=station.lower(), yy=year.astype(str)[2:]
            )
            if not _os.path.exists(filename):
                dr_station_year = selected_rnx[
                    (selected_rnx["station_name"] == station) & (selected_rnx["year"] == year)
                ]
                dr_good_station_year = dr_station_year["dr_path"][dr_station_year["good"]]
                if dr_good_station_year.shape[0] > 0:
                    print(
                        "{} good files found for {}{} out of {}. Running get_drInfo...".format(
                            dr_good_station_year.shape[0], station, year, dr_station_year.shape[0]
                        )
                    )
                    num_cores = (
                        num_cores if dr_good_station_year.shape[0] > num_cores else dr_good_station_year.shape[0]
                    )
                    with _Pool(processes=num_cores) as p:
                        if tqdm:
                            drinfo_df = _pd.concat(
                                list(
                                    _tqdm.tqdm(
                                        p.imap(_drInfo2df, dr_good_station_year),
                                        total=dr_good_station_year.shape[0],
                                        desc="{}{}".format(station.lower(), year.astype(str)[2:]),
                                    )
                                ),
                                axis=0,
                                ignore_index=True,
                            ...
                            print(
                                "Running get_drInfo for {station}{yy}.zstd".format(
                                    station=station.lower(), yy=year.astype(str)[2:]
                                )
                            )
                ...
                # gather should be separate, otherwise conflict and corrupted files
                    ...
                    print(
                        "{} good files found for {}{} out of {}. Skipping.".format(
                            dr_good_station_year.shape[0], station, year, dr_station_year.shape[0]
                        )
                    )
            else:
...
def gather_drInfo(tmp_dir, num_cores, tqdm):
    ...
    _dump_write(
        data=_pd.concat(tmp, axis=0),
        filename="{}/{}.zstd".format(rnx_dir, drInfo_lbl),
        cname="zstd",
        num_cores=num_cores,
    )
    print("gather_drInfo: Done")
    print(
        "Now run gen_tropNom() to update the tropnominals. Sites will be taken from recently generated staDb so be careful (need to change it to drInfo sites"
    )


def mode2label(mode):
    mode_table = _pd.DataFrame(
        data=[["GPS", "_g"], ["GLONASS", "_r"], ["GPS+GLONASS", "_gr"]], columns=["mode", "label"]
    )
...
def names2labels(constituents):
    constituents_labels = (
        _pd.Series(constituents).str.slice(0, 1) + "$_" + _pd.Series(constituents).str.slice(1, 2) + "$"
    )
...
def _xyz2env(dataset, reference_df, mode, dump=None):
    ...
    for i in range(len(dataset)):
        # Creating MultiIndex:
        station_name = (
            dataset[i].columns.levels[1].str.split(".", expand=True).levels[2][0].upper()
        )  # get station name from .Station.XXXX.blabla
        env_path = (
            _os.path.join(dump, "{}{}.zstd".format(station_name.lower(), mode2label(mode)))
            if dump is not None
            else None
        )
        ...
        if dump is not None and _os.path.exists(env_path):
            ...
            m_index = _pd.MultiIndex.from_product(
                [[station_name], ["value", "nomvalue", "sigma"], ["east", "north", "up"]]
            )
            ...
            refxyz = get_xyz_site(
                reference_df, station_name
            )  # stadb values. Median also possible. Another option is first 10-30% of data
            ...
            frame = _pd.DataFrame(
                _np.column_stack([diff_env_value, diff_env_nomvalue, env_sigma]), columns=m_index
            ).set_index(dataset[i].index)
...
def get_xyz_site(staDb_ref_xyz, site_name):
    ...
    return (
        staDb_ref_xyz[staDb_ref_xyz["Station"] == site_name][["X", "Y", "Z"]].squeeze().values
    )  # Squeeze to series. Not to create array in array
...
def get_const_df(ConstellationInfoFile, constellation):
    ...
    for i in range(df_SV.shape[0] - 1):
        daily_count[(daily_date >= df_SV["begin"].values[i]) & (daily_date < df_SV["begin"].values[i + 1])] = df_SV[
            "count"
        ][i]
...
def _blq2blq_df_slow(blq_file):
    ...
    for blq_record in blq:
        ...
        blq_record_df.index = _pd.MultiIndex.from_product(
            [[blq_record[0]], ["M2", "S2", "N2", "K2", "K1", "O1", "P1", "Q1", "MF", "MM", "SSA"]]
        )
    ...
    df_std = _pd.DataFrame(
        0,
        index=df.index,
        columns=_pd.MultiIndex.from_product([["OTL"], ["up", "east", "north"], ["amplitude", "phase"], ["std"]]),
    )
...
def blq2blq_df(blq_file, XYZ=False):
    ...
    if XYZ:
        components = ["X", "Y", "Z"]  # as in GipsyX com file (based on blq)
    values = _pd.concat(
        buf,
        axis=1,
        keys=_pd.MultiIndex.from_product([["OTL"], ["amplitude", "phase"], components, ["value"]]).swaplevel(i=2, j=1),
    )
    sigmas = _pd.DataFrame(
        _np.zeros(values.shape),
        index=values.index,
        columns=_pd.MultiIndex.from_product([["OTL"], ["amplitude", "phase"], components, ["std"]]).swaplevel(i=2, j=1),
    )
...
def norm_table(blq_df, custom_blq_path, normalize=True, gps_only=False, ascomplex_xy=False):
    ...
    if not custom_blq_path:
        ...
        if file_ext == ".blq":
            ...
            print("{} is unknown filetype".format(file_ext))

        stations_present_input = blq_df.index.levels[
            0
        ]  # stores a list of stations present in the input eterna analisys df
        stations_present_custom = blq_df_custom.index.levels[
            0
        ]  # stores a list of stations present in custom blq file specified
...
def check_date_margins(begin, end, years_list):
    ...
    if begin is not None:
        ...
        print("Overriding {} begin with {}".format(begin_data, begin))
        if begin < begin_data:
    ...
    if end is not None:
        ...
        print("Overriding {} end with {}".format(end_data, end))
    # if end > end_data:
...
def ECDF(x_series):
    ...
    nobs = x.shape[0]
    y = _np.linspace(1.0 / nobs, 1, nobs)
    x = x.to_frame()
    x["ECDF"] = y
    return x
...
def struct_rnx(src_files_glob="/array/bogdanm/GNSS_data/gnss-ga/*.gz", out_dir=False):
    ...
    names.name = "NAME"
    names_info = names.str.extract(
        r"(\w{4})(\d{3})\w{1}\.(\d{2})(.+)"
    )  # cannot use (\d{4}) as some files may have 'a', not '0'
