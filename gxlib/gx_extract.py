...
def _filter_tdps(tdps,std_coeff=3):
    ...
    for i in range(len(tdps)):
        ...
        filt1_data = tdp [
            #Sigma X
            (tdp['sigma'][0]<=(_np.median(tdp['sigma'][0])+ std_coeff*_np.std(tdp['sigma'][0])))&
            #Sigma Y
            (tdp['sigma'][1]<=(_np.median(tdp['sigma'][1])+ std_coeff*_np.std(tdp['sigma'][1])))&
            #Sigma Z
            (tdp['sigma'][2]<=(_np.median(tdp['sigma'][2])+ std_coeff*_np.std(tdp['sigma'][2])))
            # &
            # #Sigma X
            # (tdp.iloc[:,10]<=1)&
            # #Sigma Y
        ...
        filt2_data = filt1_data [
            #Value X
            ((filt1_data['value'][0]<=(_np.median(filt1_data['value'][0])+ std_coeff*_np.std(filt1_data['value'][0])))
                                    &(filt1_data['value'][0]>=(_np.median(filt1_data['value'][0])- std_coeff*_np.std(filt1_data['value'][0]))))
            &
            #Value Y
            ((filt1_data['value'][1]<=(_np.median(filt1_data['value'][1])+ std_coeff*_np.std(filt1_data['value'][1])))
                                    &(filt1_data['value'][1]>=(_np.median(filt1_data['value'][1])- std_coeff*_np.std(filt1_data['value'][1]))))
            &
            #Value Z
            ((filt1_data['value'][2]<=(_np.median(filt1_data['value'][2])+ std_coeff*_np.std(filt1_data['value'][2])))
                                    &(filt1_data['value'][2]>=(_np.median(filt1_data['value'][2])- std_coeff*_np.std(filt1_data['value'][2]))))
            ]
        #Step 3. Second pass over sigma values
        filt3_data = filt2_data[
            #Sigma X
            (filt2_data['sigma'][0]<=(_np.median(filt2_data['sigma'][0])+ std_coeff*_np.std(filt2_data['sigma'][0])))&
            #Sigma Y
            (filt2_data['sigma'][1]<=(_np.median(filt2_data['sigma'][1])+ std_coeff*_np.std(filt2_data['sigma'][1])))&
            #Sigma Z
            (filt2_data['sigma'][2]<=(_np.median(filt2_data['sigma'][2])+ std_coeff*_np.std(filt2_data['sigma'][2])))
            ]
