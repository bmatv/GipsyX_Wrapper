...
hatanaka=True
tree_options = trees_options.rw_no_otl
...
tree_options_code = 'trees_options.rw_no_otl'
tqdm=False
ElDepWeight = 'SqrtSin'
...
project_name_construct = _project_name_construct(project_name=project_name,PPPtype=PPPtype,pos_s=pos_s,wetz_s=wetz_s,tropNom_input=tropNom_input,ElMin=ElMin,ambres=ambres,tree_options=tree_options)
...
for i in range(len(stations_list_arrays)):
    ...
    qsub_python_code(code,name='{}{}_AR{}'.format(project_name,str(i),ElMin if ElMin!=7 else ''),email='bogdan.matviichuk@utas.edu.au',cleanup=False,pbs_base = pbs_base,walltime='06:00:00')
